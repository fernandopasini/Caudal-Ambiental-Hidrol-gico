...
# Versión corregida: carga de información + botón "Correr análisis"
#
# Entrada esperada:
# - CSV, XLSX o XLS
...
def classificar_epoca(mes: int, epocas: Dict[str, List[int]]) -> str:
...
def validar_epocas(epocas: Dict[str, List[int]]) -> Tuple[bool, List[str]]:
    todos = []
    for meses in epocas.values():
        todos.extend(meses)
    faltantes = sorted(set(range(1, 13)) - set(todos))
    repetidos = sorted([m for m in set(todos) if todos.count(m) > 1])
    mensajes = []
    if faltantes:
        mensajes.append(f"Meses sin clasificar: {faltantes}.")
    if repetidos:
        mensajes.append(f"Meses repetidos en más de una época: {repetidos}.")
...
def tabela_permanencias(q, grupo: str) -> pd.DataFrame:
    return pd.DataFrame(
        {
            "Grupo": grupo,
...
def medias_moveis_minimas_anuais(df_diario: pd.DataFrame, janela: int, min_dias_ano: int = 300) -> pd.DataFrame:
    ...
    for ano, g in aux.groupby("Ano"):
        if g["Q"].count() >= min_dias_ano:
            ...
            if pd.notna(val):
                resultados.append({"Ano": int(ano), f"Min_Q{janela}": float(val)})
...
def calcular_qjt(
    ...
    janela: int = 7,
    tempos_retorno: Optional[List[int]] = None,
    distribuicao: str = "gumbel",
    min_dias_ano: int = 300,
...
def metodo_tennant(q_media: float) -> pd.DataFrame:
    categorias = [
...
def metodo_hoppe(df_diario: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "Método": "Hoppe simplificado",
                "Componente ecológico": "Actividades diarias",
                "Permanencia (%)": 80,
                "Q (m3/s)": vazao_permanencia(df_diario["Q"], 80),
            },
            {
                "Método": "Hoppe simplificado",
                "Componente ecológico": "Desove / reproducción",
                "Permanencia (%)": 40,
                "Q (m3/s)": vazao_permanencia(df_diario["Q"], 40),
            },
            {
                "Método": "Hoppe simplificado",
                "Componente ecológico": "Descarga / lavado del sustrato",
                "Permanencia (%)": 17,
                "Q (m3/s)": vazao_permanencia(df_diario["Q"], 17),
            },
        ]
    )


def metodo_ngprp(df_diario: pd.DataFrame) -> pd.DataFrame:
    aux = df_diario.copy()
...
def metodo_abf(df_diario: pd.DataFrame):
    mensal = df_diario["Q"].resample("M").mean().dropna()
    clim = mensal.groupby(mensal.index.month).mean()
    if len(clim) == 0:
...
def filtro_lyne_hollick(q, alpha: float = 0.925, passes: int = 3) -> pd.Series:
    ...
    def one_pass(flow: pd.Series) -> pd.Series:
...
def indicadores_iha_basicos(df_diario: pd.DataFrame):
    q = df_diario["Q"].copy()
            ...
            extremos.append(
                {
                    "Ano": int(ano),
                    "Janela dias": janela,
                    "Minimo": float(g["valor"].min()),
                    "Maximo": float(g["valor"].max()),
                }
            )
    ...
    for ano, g in aux.groupby("Ano"):
        ...
        alto = s > q75
        bajo = s < q25
        n_alto = int(((alto) & (~alto.shift(1).fillna(False))).sum())
        n_bajo = int(((bajo) & (~bajo.shift(1).fillna(False))).sum())
        dif = s.diff().dropna()
        subidas = dif[dif > 0]
        descidas = dif[dif < 0]
        sinal = np.sign(dif)
        reversoes = int((sinal != sinal.shift(1)).sum())
        pulsos.append(
            {
                "Ano": int(ano),
                "Limiar bajo Q25": q25,
...
def teste_pettitt(x):
    ...
    p_valor = float(min(max(p_valor, 0), 1))
    return indice_cambio, K, p_valor


def preparar_indicadores_anuais_para_pettitt(df_diario: pd.DataFrame, min_dias_ano: int = 300) -> pd.DataFrame:
    dados = []
...
def aplicar_pettitt_indicadores(tab_indicadores: pd.DataFrame) -> pd.DataFrame:
    resultados = []
...
def sugerir_ano_pettitt(tab_pettitt: pd.DataFrame):
    ...
    return int(linha["Ano cambio Pettitt"]), str(linha["Indicador"]), float(linha["p-valor"]), bool(float(linha["p-valor"]) < 0.05)


def rva_simplificado(df_diario: pd.DataFrame, ano_corte: int, min_dias_ano: int = 300):
...
def calcular_todo(df: pd.DataFrame, serie: pd.Series, epocas: Dict[str, List[int]], min_dias_ano: int) -> Dict[str, Any]:
    q_media = serie.mean()
    ...
    for mes, g in df.groupby("mes"):
        tab_uruguay.append(
            {
                "Mes": int(mes),
                "Epoca": classificar_epoca(int(mes), epocas),
    ...
    tab_qjt_total = pd.concat(tab_qjt_total, ignore_index=True) if tab_qjt_total else pd.DataFrame()
    base = filtro_lyne_hollick(serie, alpha=0.925, passes=3)
    df_base = pd.DataFrame({"Q": serie, "Q_base": base}).dropna()
    bfi_global = df_base["Q_base"].sum() / df_base["Q"].sum() if len(df_base) > 0 and df_base["Q"].sum() != 0 else np.nan
    df_base["Ano"] = df_base.index.year
    df_base["Mes"] = df_base.index.month
    bfi_anual = df_base.groupby("Ano").apply(lambda x: x["Q_base"].sum() / x["Q"].sum() if x["Q"].sum() != 0 else np.nan).reset_index(name="BFI anual") if len(df_base) > 0 else pd.DataFrame()
    bfi_mensal = df_base.groupby("Mes").apply(lambda x: x["Q_base"].sum() / x["Q"].sum() if x["Q"].sum() != 0 else np.nan).reset_index(name="BFI mensual") if len(df_base) > 0 else pd.DataFrame()
    ...
    return {
        ...
        "tab_perm_global": tab_perm_global,
        "tab_perm_mensal": tab_perm_mensal,
        "tab_perm_epocas": tab_perm_epocas,
        "tab_uruguay": tab_uruguay,
        "tab_tennant": tab_tennant,
        "tab_hoppe": tab_hoppe,
        "tab_ngprp": tab_ngprp,
        "q_abf": q_abf,
        "mes_abf": mes_abf,
        ...
        "tab_abf": tab_abf,
        "tab_qjt_total": tab_qjt_total,
        "df_base": df_base,
        "tab_bfi_global": tab_bfi_global,
        "bfi_anual": bfi_anual,
        "bfi_mensal": bfi_mensal,
        "iha_mensais": iha_mensais,
        "iha_extremos": iha_extremos,
        "iha_pulsos": iha_pulsos,
        "tab_indicadores_pettitt": tab_indicadores_pettitt,
        "tab_pettitt": tab_pettitt,
...
def construir_resumo(serie: pd.Series, q_media: float, tab_uruguay: pd.DataFrame, q_abf: float, tab_qjt_total: pd.DataFrame) -> pd.DataFrame:
    resumo = pd.DataFrame(
        [
    ...
    if tab_qjt_total is not None and len(tab_qjt_total) > 0:
        ...
        if len(q710) > 0 and not np.isnan(q710.iloc[0]["Q (m3/s)"]):
            resumo = pd.concat(
                [
                    resumo,
                    pd.DataFrame([{"Grupo": "Mínimos móviles", "Método": "Q7,10 Gumbel", "Q eco (m3/s)": q710.iloc[0]["Q (m3/s)"], "Lectura didáctica": "Mínima media de 7 días con período de retorno de 10 años"}]),
                ],
                ignore_index=True,
            )
    return resumo
...
def exportar_excel(info_local, info_serie, resultados, resumo, guia, tab_rva, criterio_ano_corte):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        resultados["bfi_mensal"].to_excel(writer, sheet_name="13_bfi_mensal", index=False)
...
epocas_ok, mensajes_epocas = validar_epocas(epocas)
if not epocas_ok:
    for msg in mensajes_epocas:
...
st.markdown("""
Después de cargar el archivo y completar los parámetros, presione **Correr análisis**.
El aplicativo no ejecuta los cálculos hasta que se presione el botón.
""")

run_disabled = uploaded_file is None or not epocas_ok
col_run1, col_run2 = st.columns([1, 3])
with col_run1:
    correr = st.button("▶️ Correr análisis", type="primary", disabled=run_disabled)
with col_run2:
    if uploaded_file is None:
        st.info("Cargue un archivo para habilitar el análisis.")
    elif not epocas_ok:
        st.warning("Corrija la clasificación de meses en las tres épocas para habilitar el análisis.")
...
if correr:
    try:
        ...
        df["Epoca"] = df["mes"].apply(lambda m: classificar_epoca(int(m), epocas))
        ...
        with st.spinner("Calculando metodologías..."):
            resultados = calcular_todo(df, serie, epocas, int(min_dias_ano))
        ...
        st.session_state["min_dias_ano"] = int(min_dias_ano)
...
epocas = st.session_state["epocas"]
min_dias_ano = st.session_state["min_dias_ano"]
...
st.header("2. Test de Pettitt y selección del año de corte para RVA")
st.markdown("""
...
if ano_pettitt is not None:
    st.info(f"Pettitt sugiere el año {ano_pettitt}, asociado al indicador {indicador_pettitt}. p-valor = {p_pettitt:.4f}. Significativo al 5%: {'sí' if sig_pettitt else 'no'}.")
else:
...
opcion_corte = st.radio("Seleccione el criterio para el RVA", opciones_corte, key="criterio_rva")
...
if opcion_corte == "Usar año sugerido por Pettitt" and ano_pettitt is not None:
    ...
    default_manual = int(ano_medio) if ano_medio is not None else min_ano
    ano_corte = st.number_input("Año de corte manual", min_value=min_ano, max_value=max_ano, value=default_manual, step=1, key="ano_corte_manual")
...
tab_rva, _ = rva_simplificado(df, int(ano_corte), min_dias_ano=int(min_dias_ano))
...
with tab1:
    ...
    st.markdown("Los valores más bajos suelen representar criterios más restrictivos o condiciones críticas; los valores más altos tienden a mantener más agua en el río.")
...
excel_buffer = exportar_excel(
    ...
    guia=guia,
    tab_rva=tab_rva,
    criterio_ano_corte=criterio_ano_corte,
)
